...
在这三个原语上叠出五层运动：**基础编排**（位移/倾斜/挤压，`build_qwen_spritesheet.MOTION` 的关键帧按相位插值加密）+ **二级运动**（从运动自身速度取导数，让发梢、裙摆滞后于身体，髋部为支点）+ **关节弹簧**（`rigor.Spring` 跨整条循环预积分两遍再录制，头部反向稳住视线、裙摆滞后回弹；枢轴每帧从形变后的轮廓自动取，所以跟着倾斜走）+ **呼吸**（安静态胸腔纵向 ±1.1%）+ **眨眼**（每个循环里压扁眼带两帧）。
